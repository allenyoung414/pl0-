...
void gen(string func, int L, int a)//生成目标代码
{
...
//判断是否是一个const赋值<const> → <id>:=<integer>，它会被condecl_analyse()调用
bool const_analyse()
{
...
//body分析 <body> → begin <statement>{;<statement>}end他会调用statement_analyse();
bool body_analyse()
{
	int now_num = analyse_num;
...
//proc 分析 <proc> → procedure <id>（[<id>{,<id>}]）;<block>{;<proc>}他会调用block_analyse(now_table, level);
...
//变量声明<vardecl> → var <id>{,<id>};
bool vardecl_analyse()//变量声明
{
	int now_num = analyse_num;
	int flag = 0;
...
//block分析 <block> →[<condecl>][<vardecl>][<proc>]<body>
...
//语法分析，程序分析<prog> → program <id>；<block>
bool prog_analyse()
{
	int now_num = analyse_num;
